...
function getCompreFaceConfig() {
  ...
  const verifyPath = process.env.COMPRE_FACE_VERIFY_PATH || "/api/v1/recognition/verify";
...
async function compareFace(selfieBytes, galleryBytes) {
  ...
  for (const path of verifyPaths) {
    ...
    if (/api key .* not found/i.test(String(message))) {
      throw new Error(
        `CompreFace API key is not linked to a recognition service at ${verifyUrl}. ${message}`
      );
    }
  ...
  throw new Error(
    `CompreFace verification endpoint not reachable. Tried: ${attemptErrors.join(" | ")}`
  );
}
